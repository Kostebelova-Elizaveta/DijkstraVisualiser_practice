...
public class App implements Initializable {
        ...
        if(filePath==null){
            ...
            try {
                ...
                if (isGraphCreated) {
                    ...
                    loggerInstance.printMessage(getClass().toString(), "Cannot save uncreated graph!", true);
                }
            } catch (IOException ex) {
                loggerInstance.printMessage(getClass().toString(), "Cannot save graph to "+filePath, true);
            }
        }
    }
    ...
    private void saveGraphAs() {
        ...
        FileChooser.ExtensionFilter extensionFilter = new FileChooser.ExtensionFilter("Graph files (*"+fileType+")", "*"+fileType);
        ...
        if (path != null) {
            try {
                IOGraph saver = new IOGraph();
                if (isGraphCreated) {
                    saver.saveGraph(fixFileName(path), graphPane.getGraph());
                } else {
                    loggerInstance.printMessage(getClass().toString(), "Cannot save uncreated graph!", true);
                }
                filePath = fixFileName(path);
            } catch (IOException ex) {
                loggerInstance.printMessage(getClass().toString(), "Cannot save graph to "+fixFileName(path), true);
            }
        }
    }
    ...
    private void loadGraph() {
        ...
        if (path != null) {
            ...
            try {
                ...
                properties.setNeedArrows(newGraph instanceof DirectedGraphList);
                properties.setNeedDoubleEdges(newGraph instanceof DirectedGraphList);
                ...
                loggerInstance.printMessage(getClass().toString(), "Cannot open graph "+path.toString(), true);
            }
            if (isGraphCreated) {
    ...
    private void openRepo() {
        try {
            Desktop.getDesktop().browse(new URI("https://github.com/ilya201232/StudyPract"));
    ...
    public void setSettings(int playSpeed, double zoomStep, double zoomMax, double zoomMin, int typeOfSize) {
        ...
        // TODO:: реализовать тут изменение параметров (вызвать соответствующие методы для изменения)
    }
    ...
    private void openAbout() throws IOException {
        ...
        if (!isAboutOpen) {
            ...
            FXMLLoader aboutFxmlLoader = new FXMLLoader(getClass().getResource("/ru/etu/studypract/about.fxml"));
    ...
    private String fixFileName(File path){
        ...
        if(!pathStr.endsWith(fileType)&&!OS.contains("win")){
            //loggerInstance.printMessage(getClass().getName(), "Saved to directory "+pathStr+fileType);
            return pathStr+fileType;
        }
        //loggerInstance.printMessage(getClass().getName(), "Saved to directory "+pathStr);
        return pathStr;
    }
}
